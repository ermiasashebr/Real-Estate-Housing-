...
import (
	...
	"strings"
	"time"
	"github.com/ermiasashebr/housing/allEntityActions/order"
...
func (oh *OrderHandler) OrdersDelete(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodGet {
		idRaw := r.URL.Query().Get("id")
		log.Println("Orderdeleteiddddddddddddd", idRaw)
		id, err := strconv.Atoi(idRaw)
		if err!= nil {
			panic(err)
		}
