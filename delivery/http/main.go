...
import (
	...
	orepim "github.com/ermiasashebr/housing/allEntityActions/order/repository"
	osrvim "github.com/ermiasashebr/housing/allEntityActions/order/service"

	prepim "github.com/ermiasashebr/housing/allEntityActions/propertypage/repository"
	psrvim "github.com/ermiasashebr/housing/allEntityActions/propertypage/service"
	"github.com/ermiasashebr/housing/authFiles/csrfToken"
	"github.com/ermiasashebr/housing/delivery/http/handler"
	"github.com/ermiasashebr/housing/entity"

	urepimp "github.com/ermiasashebr/housing/allEntityActions/user/repository"
	usrvimp "github.com/ermiasashebr/housing/allEntityActions/user/service"
)
...
func createTabels(dbconn *gorm.DB) []error {
	errs := dbconn.CreateTable(&entity.User{}, &entity.Session{}, &entity.Property{}, &entity.Category{}, &entity.Order{}, &entity.Role{}, &entity.Feedback{}).GetErrors()
	if errs != nil {
		return errs
	}
	...
	if !dbconn.HasTable(&entity.Feedback{}) {
		errs := dbconn.CreateTable(&entity.Feedback{}).GetErrors()
		if errs != nil {
			return errs
		}
		return nil
...
func main()  {
	...
	http.Handle("/admin/users/delete", uh.Authenticated(uh.Authorized(http.HandlerFunc(uh.UsersDelete))))
	http.Handle("/admin/categories", uh.Authenticated(uh.Authorized(http.HandlerFunc(ach.AdminCategories))))
	http.Handle("/admin/properties", uh.Authenticated(uh.Authorized(http.HandlerFunc(uh.SellerPropertiesAdmin))))
	http.Handle("/admin/feedbacks", uh.Authenticated(uh.Authorized(http.HandlerFunc(uh.FeedbacksAdmin))))
	...
	http.Handle("/user/feedbacks", uh.Authenticated(uh.AuthorizedUser(http.HandlerFunc(uh.UserFeedbackDetail))))
	http.Handle("/user/feedbacksNew", uh.Authenticated(uh.AuthorizedUser(http.HandlerFunc(uh.FeedbacksNew))))
	http.Handle("/user/feedbacks/update", uh.Authenticated(uh.AuthorizedUser(http.HandlerFunc(uh.FeedbacksUpdate))))
	http.Handle("/user/feedbacks/delete", uh.Authenticated(uh.AuthorizedUser(http.HandlerFunc(uh.FeedbacksDelete))))
